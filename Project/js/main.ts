...
$('#button').click(function() : void{
  //Gets the title and year that the user inputs into the text boxes
  var title: string = $("#movieName").val(); 
  var year: string = $("#movieYear").val();   
  getMovie(title, year);
...
function getMovie(title,year) : void{
  $.ajax({
    type: 'GET',
    ...
    success: function(data) : void{
      if (data.Response == "False"){ //Displays an error message, if no such movie was found and allows user to try again.
        ...
        var starRating: number = data.imdbRating / 2;
        ...
        if (data.Poster != null){
          var moviePoster: string = data.Poster; 
